...
Goal: implement exactly one plan. Assumes fresh session.

1. Load `.ai/plans/active/P-###-*.md` + `AGENTS.md` + `.ai/map.md`. Load the cards the plan names. Nothing else up front.
...
5. Tick each checklist item in the plan file as it completes.
...
7. Finish: tell user to run `/sync`. Do not move the plan yourself.
